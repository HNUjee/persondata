...
<body class="childrenBody">
	<blockquote class="layui-elem-quote news_search">
		<div class="layui-inline">
			<a class="layui-btn layui-btn-normal newsAdd_btn">新建分类</a>
		</div>
		<div class="layui-inline">
	...
	$(function(){
		layui.config({
			...
			//加载页面数据
			var newsData = '';
			$.get(g_rootPath+"/contactType/getCategory", function(data){
				var newArray = [];
				    //正常加载信息
					newsData = data;
			...
			$(window).one("resize",function(){
				$(".newsAdd_btn").click(function(){
					var index = layui.layer.open({
						title : "添加类型",
						type : 2,
						content : g_rootPath+'/contactType/add',
				...
				if($checkbox.is(":checked")){
					layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
			            ...
			            setTimeout(function(){
			            	...
			            	//发送到服务器
			            	//$.post(g_rootPath+"/contactType/deletesCategory?ids="+ids);
			            	$.ajax({
								url:g_rootPath+"/contactType/deletesCategory",
								type : "post",
								dataType : "json",
								data:{ids:ids},
			...
			$("body").on("click",".news_edit",function(){ 
				...
				var index = layui.layer.open({
					title : "编辑文章",
					type : 2,
					content : g_rootPath+"/contactType/edit?id="+id,
			...
			$("body").on("click",".news_del",function(){ 
				...
				layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
					var id=$this.attr("data-id");
					$.ajax({
						url:g_rootPath+"/contactType/deleteCategory",
						type : "post",
						dataType : "json",
						data:{id:id},
						success : function(data){
